...
export function IntegrationPanel({
  ...
  return (
    <div className={styles.editorLayout}>
      <aside aria-label="Integrations" className={styles.automationList}>
        <button className={styles.newButton} onClick={createNew} type="button"><Icon name="new-chat" size={14} /> New integration</button>
        {integrations.map((integration) => <button aria-pressed={selectedId === integration.id && !creating} className={selectedId === integration.id && !creating ? styles.selectedAutomation : ""} key={integration.id} onClick={() => choose(integration)} type="button"><span aria-hidden="true" className={integration.enabled ? styles.enabledDot : styles.disabledDot} /><div><strong>{integration.name}</strong><span>{integration.kind} · {integration.last_test_status ?? "not tested"}</span></div></button>)}
      </aside>
