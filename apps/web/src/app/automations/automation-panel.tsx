...
export function AutomationPanel({
  ...
  const initial =
    automations.find((item) => item.id === initialAutomationId) ?? automations[0] ?? null;
  const [selectedId, setSelectedId] = useState<string | null>(initial?.id ?? null);
  const [draft, setDraft] = useState<AutomationDraft>(
    initial ? draftFromAutomation(initial) : defaultDraft,
  );
  ...
  async function action(name: "run" | "delete" | "rotate") {
    ...
    if (
      name === "delete" &&
      !window.confirm(`Delete ${selected.name} and its complete run history?`)
    ) {
      return;
    }
    setBusy(name);
  ...
  function addDelivery() {
    ...
    const channel =
      first.kind === "smtp" ? "email" : first.kind === "caldav" ? "calendar" : "webhook";
    setDraft({
      ...draft,
      deliveries: [
        ...draft.deliveries,
  ...
  function changeDeliveryIntegration(index: number, integrationId: string) {
    ...
    const channel =
      integration.kind === "smtp"
        ? "email"
        : integration.kind === "caldav"
          ? "calendar"
          : "webhook";
        ...
        {!creating && selected ? (
          <section className={styles.runSection}>
            ...
            <div className={styles.runList}>
              {runs
                .filter((run) => run.automation_id === selected.id)
                .slice(0, 8)
                .map((run) => (
                  <article key={run.id}>
                    <div>
