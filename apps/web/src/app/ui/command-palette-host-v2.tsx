...
const STATIC_ITEMS: readonly PaletteItem[] = [
  ...
  },
  {
    ...
    group: "Workspace",
    label: "Email",
    description: "Open the full-size email reader",
    icon: "email",
    keywords: "email inbox sent drafts archive spam junk trash mail",
    action: { type: "route", href: "/email" },
  },
  {
    id: "workspace:connections",
    group: "Workspace",
    label: "Connections",
    description: "Configure Discord, MCP, calendars, SMTP, and webhooks",
    icon: "tools",
    keywords: "connections discord channels bot mcp smtp caldav webhook integrations",
    action: { type: "route", href: "/connections" },
  },
  {
    id: "workspace:skills",
    group: "Workspace",
  ...
  },
  {
    ...
    group: "Workspace",
    label: "Tasks",
    description: "Open built-in tasks, custom schedules, and run history",
    icon: "refresh",
    keywords: "tasks scheduled jobs workflows runs automations",
...
function conversationItem(conversation: ConversationSummary): PaletteItem {
  const messageLabel = `${conversation.message_count} message${
  ...
  return {
    ...
    description: conversation.persona_name
      ? `${messageLabel} · ${conversation.persona_name}`
      : messageLabel,
    icon: "chat",
...
export function CommandPaletteHostV2() {
  ...
  const closePalette = useCallback((restoreFocus = true) => {
    ...
    setSelectedIndex(0);
    if (restoreFocus) requestAnimationFrame(() => lastFocusedRef.current?.focus());
  }, []);
