...
const WORKSPACE_ITEMS: readonly {
  ...
  { key: "communications", href: "/communications", icon: "chat", label: "Communications" },
  { key: "agents", href: "/agents", icon: "tools", label: "Agents" },
  { key: "images", href: "/images", icon: "images", label: "Images" },
  { key: "automations", href: "/automations", icon: "refresh", label: "Automations" },
];

type ActiveKey = "chat" | WorkspaceWindowKey | "settings" | null;
...
export function ApplicationSidebarHost() {
  ...
  function openSettings(section?: "account") {
    window.dispatchEvent(
      new CustomEvent(OPEN_SETTINGS_EVENT, { detail: section ? { section } : undefined }),
    );
    closeMobile();
  }

  function openWorkspace(event: MouseEvent<HTMLAnchorElement>, key: WorkspaceWindowKey) {
    ...
    event.preventDefault();
    window.dispatchEvent(
      new CustomEvent(OPEN_WORKSPACE_WINDOW_EVENT, { detail: { window: key } }),
    );
    closeMobile();
  }
